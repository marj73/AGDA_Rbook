...
-   *Exploratory Data Analysis and Visualization*: examining geographical variations in statistical data distributions, including Geographically Weighted Summary Statistics ([[Chapter 3](\@ref(gwss))]{#03-GWSS}).
-   *Cluster Detection and Mapping*: global clustering approaches--namely Ripley's K-function and Kernel Density Estimator ([Chapter 4](\@ref(st-cluster))) -- and an example of local method, DBSCAN, for identifying and visualizing clustering features ([Chapter 8](\@ref(dbscan))).
-   *Random Forest*: as example of supervised machine learning algorithm. We present both the global ([Chapter 5](\@ref(rf-cl))) and the local version, and we use Random Forest to introduce interpretability & explainability in machine learning ([Chapter 6](\@ref(rf-exp))) .
-   *Self-Organizing Maps*: as example of unsupervised machine learning method for data clustering/segmentation and visualization ([Chapter 7](\@ref(som))).

In addition, the first two chapters offer a brief introduction to the main functionalities in R [Chapter 1](@ref(intro-r)) and cover some basic operations with geospatial data in R [Chapter 2](@ref(r-gis)).
