...
-   *Cluster Detection and Mapping*: global clustering approaches--namely Ripley's K-function and Kernel Density Estimator ([Chapter 4](#04-KDE))--and an example of local method, DBSCAN, for identifying and visualizing clustering features ([Chapter 8](#08-DBSCAN)).
-   *Random Forest*: as example of supervised machine learning algorithm. We present both the global [(Chapter 5)](#05-RF) and the local version ([Chapter 6](#06-Exp_RF)), and we use Random Forest to introduce interpretability & explainability in machine learning ([Chapter 6](#06-Exp_RF)) .
-   *Self-Organizing Maps*: as example of unsupervised machine learning method for data clustering/segmentation and visualization ([Chapter 7](#08-DBSCAN)).

In addition, the first two chapters offer a brief introduction to the main functionalities in R ([Chapter 1](#01-IntroR)) and cover some basic operations with geospatial data in R ([Chapter 2](#02-RGIS)).
